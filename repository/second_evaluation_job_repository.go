...
func IsAllFirstPassUserHaveAppliedScreening(db *gorm.DB) (bool, error) {
	...
	if err != nil {
		return false, e.WrapRollbackNeededError(err)
	}
...
func IsAllAbsenteeFall(db *gorm.DB) (bool, error) {
	...
	err = db.Raw(query).Scan(&fallCount).Error
	if err != nil {
		return false, e.WrapRollbackNeededError(err)
	}
...
func UpdateSecondTestPassYnForExtraAdPass(passExtraAdOneseoIds []int, db *gorm.DB) error {
	...
	query := `
		...
		`
	err := db.Exec(query, passExtraAdOneseoIds).Error
	if err != nil {
		return e.WrapRollbackNeededError(err)
	}
...
func UpdateAppliedScreeingForExtraAdFall(fallExtraAdOneseoIds []int, db *gorm.DB) error {
	query := `
			...
			WHERE oneseo_id IN ?
		`
	err := db.Exec(query, fallExtraAdOneseoIds).Error
	if err != nil {
		return e.WrapRollbackNeededError(err)
	}
...
func UpdateSecondTestPassYnForExtraVePass(passExtraVeOneseoIds []int, db *gorm.DB) error {
	...
	query := `
			...
			WHERE tbo.oneseo_id IN ?
		`
	err := db.Exec(query, passExtraVeOneseoIds).Error
	if err != nil {
		return e.WrapRollbackNeededError(err)
	}
...
func UpdateAppliedScreeingForExtraVeFall(fallExtraVeOneseoIds []int, db *gorm.DB) error {
	query := `
			...
			WHERE oneseo_id IN ?
		`
	err := db.Exec(query, fallExtraVeOneseoIds).Error
	if err != nil {
		return e.WrapRollbackNeededError(err)
	}
...
func UpdateSecondTestPassYnForSpecialPass(passSpecialOneseoIds []int, db *gorm.DB) error {
	...
	query := `
			...
			WHERE tbo.oneseo_id IN ?
		`
	err := db.Exec(query, passSpecialOneseoIds).Error
	if err != nil {
		return e.WrapRollbackNeededError(err)
	}
...
func UpdateAppliedScreeningForSpecialFall(fallSpecialOneseoIds []int, db *gorm.DB) error {
	query := `
			UPDATE tb_oneseo
			SET applied_screening = 'GENERAL'
			WHERE oneseo_id IN ?
		`
	err := db.Exec(query, fallSpecialOneseoIds).Error
	if err != nil {
		return e.WrapRollbackNeededError(err)
	}
